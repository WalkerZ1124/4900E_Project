...
// This function is used to corrupt the firmware image in the partition table
static void corrupt_firmware(const esp_partition_t *partition) {
    uint8_t corrupted_data[32]; // Corrupt only the header
    memset(corrupted_data, 0x00, sizeof(corrupted_data)); // Fill with invalid data

    esp_err_t err = esp_partition_write(partition, 0, corrupted_data, sizeof(corrupted_data));
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to write corrupted data: %s", esp_err_to_name(err));
    } else {
